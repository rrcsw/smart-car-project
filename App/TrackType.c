...
void LoopIntoRepair()
{
   ...
   if(LoopExit.RightControlExit)//如果获得了出口在右边的控制信号
   {
       if(LoopExit.IntoLoopOk||LoopExit.LoopInFlag)
       {
        for(i=58;i>LastLine;i--)
        {   
         MiddleLine[i]=RightEdge[i]-LoopWidth[i]/2-2;//右边线-赛道半宽
        }
       ...
       else
       {
         for(i=58;i>LastLine;i--)
        {
         MiddleLine[i]=RightEdge[i]-LoopWidth[i]/3-1;//右边线-赛道半宽
   ...
   else if(LoopExit.LeftControlExit)//如果获得了出口在左边的控制信号
   {
     if(LoopExit.IntoLoopOk||LoopExit.LoopInFlag)
     {
       for(i=58;i>LastLine;i--)
       {
          MiddleLine[i]=LeftEdge[i]+LoopWidth[i]/2+2;//左边线+赛道半宽
     ...
     else
     {
        for(i=58;i>LastLine;i--)
       {
          MiddleLine[i]=LeftEdge[i]+LoopWidth[i]/3-1;//左边线+赛道半宽
